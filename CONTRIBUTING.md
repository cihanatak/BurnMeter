...
normalized `UsageRecord` (see `ccmeter/parser.py`), so a new agent only needs a
...
1. **Write `ccmeter/<agent>_parser.py`** with a `load_<agent>_records(root,
...
2. **Add pricing** in `ccmeter/pricing.py`: a `ModelPrice` entry per model family
...
3. **Wire the source** in `ccmeter/server.py` (`pick_cache` + a `_Cache` with a
   `worker_config`) and `ccmeter/_worker.py` (a branch for your `source`). Add the
...
## Dev loop
```bash
python3 -m ccmeter serve --port 9876        # run
node --check static/dashboard.js            # JS sanity
python3 -c "import ccmeter.server"          # import sanity
echo '{"source":"codex"}' | python3 -m ccmeter._worker | python3 -m json.tool | head
```
