# ccmeter Sync — çok-cihaz (Pro) mimarisi
...
- **Bağımlılık:** `cryptography` (Fernet/AES). YALNIZCA sync modülünde **lazy import** → free çekirdek sıfır-dep kalır. (`pip install ccmeter[sync]` extra'sı.)
...
## 4. İstemci (ccmeter tarafı)

Yeni CLI: `ccmeter sync`
- `ccmeter sync login` — account_token + passphrase'i lokal config'e yazar (`~/.config/ccmeter/sync.json`, 0600).
- `ccmeter sync push` — bu cihazın snapshot'ını şifreleyip relay'e PUT. (serve içinde arka planda periyodik de çağrılır.)
- `ccmeter sync pull` — relay'den tüm cihaz ciphertext'lerini çek, deşifre et, lokal cache'e yaz.
- `ccmeter sync status` — bağlı cihazlar, son sync, plan.
...
- **F1 — Transport MVP (lokal test):** relay (self-host) + `ccmeter sync push/pull/status` + E2E şifreleme + stub Pro-gate. Lokal: relay başlat → cihaz A push → cihaz B pull → dashboard'da 2. cihaz görünür. **Demoable, dış bağımlılık yok.**
...
2. **Şifreleme bağımlılığı:** Pro modülünde `cryptography` (E2E için) — free çekirdek sıfır-dep kalır, `ccmeter[sync]` extra'sı [ÖNERİLEN]. Onaylıyor musun?
