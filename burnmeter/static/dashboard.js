...
function initModularGrid() {
  ...
  window.__grid = grid;
  try {
    const saved = JSON.parse(localStorage.getItem("burnmeter_layout_v5") || "null");
  ...
  const save = () => { try { if (grid.getColumn() === 12) localStorage.setItem("burnmeter_layout_v5", JSON.stringify(grid.save(false))); } catch (e) {} };
  grid.on("change", save);
...
function start() {
  ...
  if (rl) rl.addEventListener("click", () => { localStorage.removeItem("burnmeter_layout_v5"); location.reload(); });
